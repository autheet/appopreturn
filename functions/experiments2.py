...
def get_unspents_resiliently(address):
    """
    Tries a list of API providers in parallel to fetch UTXOs and returns the first valid result.
    TODO: add consent logic to only use utxo that is the same between two providers.
    """
    providers = [
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(providers)) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_provider):
            ...
            try:
                ...
                if fee >= 20:
                    fee = 20
                    print(f"Fee provider {provider_func.__name__} returned a high fee. Capping at 20 sat/vB")
                fees.append(fee)
            except Exception as e:
...
def transact(private_key_string, file_digest):
    ...
    # 2. Get fees and UTXOs in parallel for efficiency
